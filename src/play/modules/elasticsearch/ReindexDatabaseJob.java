...
public class ReindexDatabaseJob extends Job<Void> {
  ...
  /**
   * Default constructor which creates a job which reindexes all entities in the database in one
   * thread.
   */
    ...
    for (final ManagedType managedType : types) {
      ...
      if (count > Integer.MAX_VALUE) {
        throw new RuntimeException(
            String.format("Number of entities %s to index is too large (%d)", modelClass, count));
      }
