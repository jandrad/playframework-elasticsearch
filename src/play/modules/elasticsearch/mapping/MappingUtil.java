...
public abstract class MappingUtil {
  ...
  private static void addIndexAndStoreInformation(XContentBuilder builder,
      ElasticSearchField fieldMeta)
      throws IOException {
  ...
  private static Date getDate(String val) {
    try {
      ...
      return ISODateTimeFormat.dateTime().withZone(DateTimeZone.UTC).parseDateTime(val).toDate();
    } catch (Throwable t) {
