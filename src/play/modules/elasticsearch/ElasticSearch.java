...
public abstract class ElasticSearch {
   ...
   */
  static <T extends Model> SearchRequestBuilder builder(final QueryBuilder query,
      final Class<T> clazz) {
  ...
  /**
   * Search with optional aggregations.
   *
   * @param <T>
   *            the generic type
   * @param query
   ...
   */
  public static <T extends Model> SearchResults<T> search(final QueryBuilder query,
      final Class<T> clazz, final AggregationBuilder... facets) {
    return search(query, clazz, false, facets);
  }

  /**
   * Search with optional aggregations. Hydrates entities
   *
   * @param <T>
   *            the generic type
   * @param query
   ...
   * @return the search results
   */
  public static <T extends Model> SearchResults<T> searchAndHydrate(final QueryBuilder query,
      final Class<T> clazz, final AggregationBuilder... facets) {
   ...
   * @return the search results
   */
  private static <T extends Model> SearchResults<T> search(final QueryBuilder query,
      final Class<T> clazz, final boolean hydrate, final AggregationBuilder... facets) {
    ...
    // Add aggregations
    for (final AggregationBuilder facet : facets) {
